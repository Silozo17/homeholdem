...
export function DealerCharacter({ className, expression = 'neutral' }: DealerCharacterProps) {
  return (
    <div className={cn('flex flex-col items-center gap-0.5', className)}>
      ...
      {/* Dealer avatar frame */}
      <div
        className="relative w-12 h-12 rounded-full flex items-center justify-center dealer-breathe"
        style={{
      ...
      >
        ...
        <Crown
          className={cn(
            'w-5 h-5 relative z-[1] transition-transform duration-500',
            expression === 'smile' && 'scale-110 text-primary',
            expression === 'surprise' && 'scale-125 text-primary',
            expression === 'neutral' && 'text-primary/80',
          )}
          style={{
            filter: 'drop-shadow(0 0 6px hsl(43 74% 49% / 0.5))',
          }}
        />
        ...
        {expression === 'smile' && (
          <>
            <Sparkles className="absolute -top-1 -right-1 w-3 h-3 text-primary animate-sparkle" />
            <Sparkles className="absolute -bottom-0.5 -left-1 w-2.5 h-2.5 text-primary animate-sparkle" style={{ animationDelay: '0.3s' }} />
          </>
        )}
      </div>
