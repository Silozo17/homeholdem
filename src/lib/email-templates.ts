...
// Mobile-first, dark casino green theme with gold accents
...
const baseStyles = {
  ...
  card: `background: linear-gradient(180deg, #172a24 0%, #0d1916 100%); border: 1px solid rgba(212, 175, 55, 0.25); border-radius: 16px; max-width: 480px; margin: 0 auto; overflow: hidden; box-shadow: 0 4px 24px rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(212, 175, 55, 0.1);`,
  header: `text-align: center; padding: 28px 16px 20px; border-bottom: 1px solid rgba(212, 175, 55, 0.15); background: linear-gradient(180deg, rgba(212, 175, 55, 0.08) 0%, transparent 100%);`,
  ...
  subtext: `color: #7a9e90; font-size: 15px; line-height: 1.5; margin: 0 0 28px;`,
  infoCard: `background: linear-gradient(135deg, rgba(212, 175, 55, 0.12) 0%, rgba(212, 175, 55, 0.06) 100%); border: 1px solid rgba(212, 175, 55, 0.25); border-radius: 12px; padding: 20px 24px; margin: 0 0 28px; text-align: left;`,
  button: `display: inline-block; background: linear-gradient(135deg, #d4af37 0%, #b8962e 100%); color: #000000; font-weight: 600; font-size: 14px; padding: 16px 40px; border-radius: 8px; text-decoration: none; text-align: center; box-shadow: 0 4px 12px rgba(212, 175, 55, 0.3);`,
  footer: `text-align: center; padding: 20px 24px; border-top: 1px solid rgba(212, 175, 55, 0.15); background: linear-gradient(0deg, rgba(212, 175, 55, 0.05) 0%, transparent 100%);`,
  footerText: `color: #3d5e52; font-size: 11px; margin: 0; letter-spacing: 1px;`,
};

function emailWrapper(content: string): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Home Hold'em Club</title></head><body style="${baseStyles.wrapper}"><div style="${baseStyles.container}"><div style="${baseStyles.card}"><div style="${baseStyles.header}"><p style="${baseStyles.headerText}">♠ Home Hold'em Club ♠</p></div>${content}<div style="${baseStyles.footer}"><p style="${baseStyles.footerText}">Home Hold'em Club</p><p style="color: #2d4a40; font-size: 14px; letter-spacing: 8px; margin-top: 8px;">♥ ♠ ♦ ♣</p></div></div></div></body></html>`;
}

export function otpTemplate({ code, name }: { code: string; name?: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="font-size: 48px; margin-bottom: 20px;">🔐</div><h1 style="${baseStyles.headline}">Verify your email</h1><p style="${baseStyles.subtext}">${name ? `Hey ${name}, enter` : 'Enter'} this code to complete your signup</p><div style="background: linear-gradient(135deg, #1f3830 0%, #172a24 100%); border: 2px solid #d4af37; border-radius: 12px; padding: 24px 20px; margin: 0 auto 24px; max-width: 220px; box-shadow: 0 0 20px rgba(212, 175, 55, 0.15);"><p style="font-size: 32px; font-weight: 700; letter-spacing: 6px; color: #d4af37; font-family: monospace; margin: 0;">${code}</p></div><p style="color: #5e8577; font-size: 12px; margin: 0;">This code expires in 10 minutes</p></div>`);
}
...
export function gameResultsTemplate({ eventTitle, placements, prizePool, yourPlacement, resultsUrl }: { eventTitle: string; eventDate: string; placements: { name: string; prize: string; position: number }[]; prizePool: string; yourPlacement?: { position: number; prize?: string }; resultsUrl: string }): string {
  const podiumEmojis = ['🥇', '🥈', '🥉'];
  const placementList = placements.slice(0, 3).map((p, i) => `<div style="display: flex; justify-content: space-between; padding: 12px 0; border-bottom: 1px solid rgba(212, 175, 55, 0.15);"><span style="color: #ffffff;">${podiumEmojis[i]} ${p.name}</span><span style="color: #d4af37; font-weight: 600;">${p.prize}</span></div>`).join('');
  const getOrdinal = (n: number) => { const s = ['th', 'st', 'nd', 'rd']; const v = n % 100; return s[(v - 20) % 10] || s[v] || s[0]; };
  return emailWrapper(`<div style="${baseStyles.content}"><div style="font-size: 48px; margin-bottom: 20px;">🏆</div><h1 style="${baseStyles.headline}">Results are in</h1><p style="${baseStyles.subtext}">${eventTitle} has concluded. Thanks for playing!</p>${yourPlacement ? `<div style="background: linear-gradient(135deg, rgba(212, 175, 55, 0.2) 0%, rgba(212, 175, 55, 0.1) 100%); border: 1px solid rgba(212, 175, 55, 0.35); border-radius: 12px; padding: 16px 20px; margin-bottom: 20px; box-shadow: 0 0 20px rgba(212, 175, 55, 0.1);"><p style="margin: 0 0 4px; color: #7a9e90; font-size: 12px; text-transform: uppercase;">Your finish</p><p style="margin: 0; color: #d4af37; font-size: 28px; font-weight: 700;">${yourPlacement.position}${getOrdinal(yourPlacement.position)} Place</p>${yourPlacement.prize ? `<p style="margin: 8px 0 0; color: #ffffff; font-size: 16px;">Won: ${yourPlacement.prize}</p>` : ''}</div>` : ''}<div style="${baseStyles.infoCard}"><p style="margin: 0 0 4px; color: #7a9e90; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">Final Standings</p>${placementList}<div style="margin-top: 16px; padding-top: 12px; border-top: 1px solid rgba(212, 175, 55, 0.25);"><span style="color: #7a9e90; font-size: 13px;">Prize Pool:</span><span style="color: #ffffff; font-weight: 600; margin-left: 8px;">${prizePool}</span></div></div><a href="${resultsUrl}" style="${baseStyles.button}">View Full Results</a></div>`);
}

export function passwordResetTemplate({ resetUrl }: { resetUrl: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="font-size: 48px; margin-bottom: 20px;">🔑</div><h1 style="${baseStyles.headline}">Reset your password</h1><p style="${baseStyles.subtext}">We received a request to reset your password. Click below to choose a new one.</p><a href="${resetUrl}" style="${baseStyles.button}">Reset Password</a><p style="color: #5e8577; font-size: 12px; margin-top: 24px;">This link expires in 1 hour. If you didn't request this, you can safely ignore this email.</p></div>`);
}
