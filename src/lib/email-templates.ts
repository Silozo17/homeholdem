...
// Mobile-first, dark casino green theme with gold accents and premium shine effects
...
const baseStyles = {
  ...
  emoji: `font-size: 48px; line-height: 1; margin-bottom: 20px; filter: drop-shadow(0 4px 8px rgba(0, 0, 0, 0.3));`,
...
export function otpTemplate({ code, name }: { code: string; name?: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">🔐</div><h1 style="${baseStyles.headline}">Verify your email</h1><p style="${baseStyles.subtext}">${name ? `Hey ${name}, enter` : 'Enter'} this code to complete your signup</p><div style="${baseStyles.codeBox}"><p style="${baseStyles.codeText}">${code}</p></div><p style="color: #6b9a8a; font-size: 12px; margin: 0;">This code expires in 10 minutes</p></div>`);
}

export function welcomeTemplate({ name, dashboardUrl }: { name: string; dashboardUrl: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">🎰</div><h1 style="${baseStyles.headline}">Welcome to the table</h1><p style="${baseStyles.subtext}">Hey ${name}, your account is ready. Create your first club or join an existing one to start organizing poker nights.</p><a href="${dashboardUrl}" style="${baseStyles.button}">Get Started</a></div>`);
}
...
export function eventCreatedTemplate({ eventTitle, clubName, description, dateOptions, eventUrl }: { eventTitle: string; clubName: string; description?: string; dateOptions: string[]; eventUrl: string }): string {
  const dateList = dateOptions.map(date => `<li style="margin-bottom: 10px; color: #ffffff; padding: 8px 12px; background: rgba(212, 175, 55, 0.08); border-radius: 6px; border-left: 3px solid #d4af37;">📅 ${date}</li>`).join('');
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">🎲</div><h1 style="${baseStyles.headline}">New poker night</h1><p style="${baseStyles.subtext}"><strong style="color: #ffffff;">${eventTitle}</strong> was created in ${clubName}.${description ? `<br><br>${description}` : ''}</p><div style="${baseStyles.infoCard}"><p style="margin: 0 0 12px; color: #8fb5a5; font-size: 12px; text-transform: uppercase; letter-spacing: 2px;">Vote on a date</p><ul style="margin: 0; padding-left: 0; list-style: none;">${dateList}</ul></div><a href="${eventUrl}" style="${baseStyles.button}">Vote & RSVP</a></div>`);
}
...
export function rsvpConfirmationTemplate({ eventTitle, status, eventDate, location, eventUrl }: { eventTitle: string; status: 'going' | 'maybe' | 'not_going'; eventDate?: string; location?: string; eventUrl: string }): string {
  const config = { 
    going: { emoji: '✅', headline: "You're in!", message: 'Your seat is confirmed.' }, 
    maybe: { emoji: '🤔', headline: 'Noted!', message: "We've recorded your maybe." }, 
    not_going: { emoji: '👋', headline: 'Maybe next time', message: "We'll miss you at the table." } 
  };
  const { emoji, headline, message } = config[status];
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">${emoji}</div><h1 style="${baseStyles.headline}">${headline}</h1><p style="${baseStyles.subtext}">${message}</p>${(eventDate || location) ? `<div style="${baseStyles.infoCard}"><p style="margin: 0 0 4px; color: #ffffff; font-weight: 600; font-size: 16px;">${eventTitle}</p>${eventDate ? `<p style="margin: 10px 0 0; color: #b8d4c8; font-size: 14px;">📅 ${eventDate}</p>` : ''}${location ? `<p style="margin: 8px 0 0; color: #b8d4c8; font-size: 14px;">📍 ${location}</p>` : ''}</div>` : ''}<a href="${eventUrl}" style="${baseStyles.button}">View Event</a></div>`);
}
...
export function eventReminderTemplate({ eventTitle, eventDate, location, hostName, eventUrl }: { eventTitle: string; eventDate: string; location?: string; hostName?: string; eventUrl: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">⏰</div><h1 style="${baseStyles.headline}">Tomorrow night</h1><p style="${baseStyles.subtext}">Don't forget – you have a poker night coming up!</p><div style="${baseStyles.infoCard}"><p style="margin: 0 0 4px; color: #ffffff; font-weight: 600; font-size: 16px;">${eventTitle}</p><p style="margin: 12px 0 0; color: #b8d4c8; font-size: 14px;">📅 ${eventDate}</p>${location ? `<p style="margin: 8px 0 0; color: #b8d4c8; font-size: 14px;">📍 ${location}</p>` : ''}${hostName ? `<p style="margin: 8px 0 0; color: #b8d4c8; font-size: 14px;">🏠 Hosted by ${hostName}</p>` : ''}</div><a href="${eventUrl}" style="${baseStyles.button}">View Details</a></div>`);
}
...
export function waitlistPromotionTemplate({ eventTitle, eventDate, location, clubName, eventUrl }: { eventTitle: string; eventDate?: string; location?: string; clubName?: string; eventUrl: string }): string {
  return emailWrapper(`<div style="${baseStyles.content}"><div style="${baseStyles.emoji}">🎉</div><h1 style="${baseStyles.headline}">A spot opened up!</h1><p style="${baseStyles.subtext}">Great news! You've been promoted from the waitlist and your seat is now confirmed.</p><div style="${baseStyles.highlightCard}"><p style="margin: 0; color: #d4af37; font-size: 13px; font-weight: 600; text-transform: uppercase; letter-spacing: 2px; text-shadow: 0 0 10px rgba(212, 175, 55, 0.3);">✨ Seat Confirmed ✨</p></div><div style="${baseStyles.infoCard}"><p style="margin: 0 0 4px; color: #ffffff; font-weight: 600; font-size: 16px;">${eventTitle}</p>${clubName ? `<p style="margin: 10px 0 0; color: #8fb5a5; font-size: 13px;">🎴 ${clubName}</p>` : ''}${eventDate ? `<p style="margin: 8px 0 0; color: #b8d4c8; font-size: 14px;">📅 ${eventDate}</p>` : ''}${location ? `<p style="margin: 8px 0 0; color: #b8d4c8; font-size: 14px;">📍 ${location}</p>` : ''}</div><a href="${eventUrl}" style="${baseStyles.button}">View Event</a></div>`);
}
